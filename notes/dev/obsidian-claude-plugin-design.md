# BojuBot — Architecture Reference
...
Session metadata lives in `.obsidian/bojubot/sessions/<id>.json`. Actual conversation history stays in `~/.claude/projects/` (managed by the Claude CLI). Session files contain:
...
The orientation block (`src/orientation.md`) is compiled into `main.js` via esbuild's text loader — it is NOT a user-accessible vault file. The `@@BOJU_ACTION` and `@@BOJU_QUERY` strings in it are an attack surface; they must never be written to user-readable vault files.

### Skills System

Skills are the primary way users extend BojuBot with custom workflows. A skill is a `.md` file in the configured skills folder (default: `_BojuBot Skills/`) or a `<name>/SKILL.md` subdirectory (Claude Code convention).

**Frontmatter fields:**

| Field            | Description                                      |
| ---------------- | ------------------------------------------------ |
| `params:`        | Array of typed form fields (BojuBot format)      |
...
**Param types (BojuBot):** `input`, `textarea`, `dropdown`, `checkboxes`, `obsidianmd_note` (alias: `note`)

The `obsidianmd_note` type opens a fuzzy vault picker; the selected note's full content is injected as a file attachment. This type is named `obsidianmd_note` (not `note`) so Claude Code and other tools can gracefully degrade when reading BojuBot skills.

**Claude Code compatibility:** BojuBot skills are a strict superset of Claude Code skill format. Skills authored for Claude Code/Cursor/Gemini CLI work in BojuBot as-is.
...
### UIBridge Protocol (@@BOJU_ACTION)

Claude triggers Obsidian UI actions by emitting `@@BOJU_ACTION {"action": "...", ...params}` on its own line in a response. These are intercepted by `src/UIBridge.ts` and never shown to the user.
...
Claude queries live vault state by emitting `@@BOJU_QUERY {"query": "...", ...params, "mode": "show"|"inject"}`.
...
Greeting is consent-based: reads `settings.userLabel`. This value is written only when the user introduces themselves via conversation and Claude fires `@@BOJU_ACTION {"action": "set-label", "name": "..."}`. The plugin never reads the OS username.
...
- Command reference file (`.obsidian/plugins/bojubot/obsidian-commands.md` — generated on layout ready)
- Log file in plugin dir (`.obsidian/plugins/bojubot/bojubot-debug.log`)
...
| Skills folder             | `_BojuBot Skills/`   | Where skill .md files live                    |
...
| `src/UIBridge.ts`               | `@@BOJU_ACTION` parsing + execution; `ConfirmCommandModal`; allowlist/denylist enforcement         |
| `src/QueryHandler.ts`           | `@@BOJU_QUERY` resolution; `resolveQuery()`, `queryLabel()`, `buildInjectMessage()`                |
...
The vision is a layered plugin architecture where BojuBot core exposes a stable API (`core.api`) that first- and third-party plugins consume:

- **`bojubot-watchers`** — filesystem event triggers that spawn Claude sessions on vault changes
- **`bojubot-supervisor`** — session queuing, concurrency control, multi-agent fleet orchestration
- **`bojubot-mcp`** — exposes vault skills as MCP tools over localhost HTTP/SSE for external clients (Claude Desktop, n8n, mobile via Tailscale)

Core Plugin API spec: `notes/dev/specs/bojubot-core-api-spec.md`

### Other Planned Features

- **Inline content generation (#10)** — `<% bojubot: {"prompt": "..."} %>` tags in notes trigger headless Claude calls; output replaces the tag. Spec: `notes/dev/specs/inline-content-generation-spec.md`
...
- **Vault watch/event system (#62)** — Obsidian pushing vault state changes to Claude proactively (second half of the two-way bridge; `@@BOJU_QUERY` is the first half).
- **Two-way bridge improvements** — `help` topic injection for on-demand reference docs, unified `@@BOJU` prefix, selection-aware context injection.
